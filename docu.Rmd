...
a common agricultural policy (CAP). In 1962, the first agricultural subsidies were introduced, reaching producers in the form of intervention prices or export refunds. Since 1992, price supports have been gradually transformed into direct
payments and disbursed in a manner decoupled from the crops (*Garzon*, 2006). 


## Current Situation

The current period is overshadowed by numerous major crises at the international level. Agriculture is also affected by this. 
Currently, the CAP is being fundamentally reformed.
The aim of the new CAP is to make agriculture more sustainable and climate-friendly. It should contribute to the set environmental and climate goals of the European Union.
...
To achieve the new goals, there will be a change in the payment of EU subsidies. Until now, only the requirements on the fertilizer regulation in Germany had to be met to receive subsidies.  Environmental requirements and thus agronomic measures were not linked to the payment of subsidies. From 2023, the "basic premium" is expected to fall from the current €170/ha to around €150/ha. Furthermore, from then on, conditions must be met for the basic premium to be paid out (*Bockholt*, 2022; *Michalczyk*, 2022).  
...
TIt states that, in the future, EU subsidies will only be paid, if the conditions are fulfilled. These conditions can severely restrict the farmers in their agronomic possibilities and can have a strong impact on the success of the farms (*Díaz* et al., 2021). 
...
As a result, the prices of the products are exposed to significant price fluctuations and are therefore unstable. Depending on the uncertainties described, mandatory measures will have to be implemented in the future in order to receive subsidies. These may have different effects on the success of the individual farms.
...
The model function is run twice. Once the receipt of subsidies is simulated and the expected profit is calculated under the premise that subsidies are paid and the necessary cuts and their costs are accepted. In the second variant, only the profit to be expected if no subsidies are paid is calculated. It should be noted that in this case there is no costs for subsidies. The exact execution of the model function can be found in the comments in the code.
